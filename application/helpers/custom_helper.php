...
//post method
if (!function_exists('positive_number')) {
	function positive_number($int, $decimals = 0)
	{
		if ($int < 0) {
			$int = $int * -1;
		}
		return number_format(html_escape($int), $decimals);
	}
}

if (!function_exists('number_words')) {
	function number_words($num = false)
	{
...
if (!function_exists('formatted_date')) {
	function formatted_date($timestamp, $format = 'd M Y')
	{
...
if (!function_exists('formatted_time')) {
	function formatted_time($timestamp, $format = 'H:i:s')
	{
		return date($format, strtotime($timestamp));
	}
}
