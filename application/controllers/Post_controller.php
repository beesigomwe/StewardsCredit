...
class Post_controller extends Admin_Core_Controller{
	...
	public function add_post() {
		...
		$data['post_type'] = "post";
		$data['contacts'] = $this->auth_model->get_users();
		$data['p_method'] = $this->post_admin_model->get_payment_methods();
		$data['accounts'] = $this->post_admin_model->get_payment_accounts();
	...
	public function add_post_post() {
		...
		$this->form_validation->set_rules('userid', 'Account holder', 'required');
		$this->form_validation->set_rules('category_id', 'Account type', 'required');
		...
		// if($userid !== $this->input->post('userid')){ // TODO: Temporarily allow for user to create accounts for themselves for Linda till we get another admin then I'll remove this 
			...
			//if post added
				...
				//add initial payment transaction
					$this->comment_model->initial_transaction($last_id);
		...
		// }else{
		// 	$this->session->set_flashdata('errors', 'Cannot post transaction for own account.');
		// 	$this->session->set_flashdata('form_data', $this->post_admin_model->input_values());
		// 	redirect($this->agent->referrer());
		// }
	}
