...
<div class="row">
		...
		<div class="row">
			<div class="col-sm-6">
				<div class="card">
					...
					<!-- /.box-header -->
					<!-- form start -->
					<?= form_open('admin_controller/add_menu_link_post'); ?>
			...
			<div class="col-sm-6">
				<div class="card">
					...
					<!-- /.box-header -->
					<!-- form start -->
					<?= form_open('admin_controller/menu_limit_post'); ?>
					...
					<div class="card-body">
						...
						<div class="form-group">
							...
							<input type="number" class="form-control" name="menu_limit" placeholder="<?= trans('menu_limit'); ?>" value="<?= $general_settings->menu_limit; ?>" min="1" max="99999" <?= ($rtl == true) ? 'dir="rtl"' : ''; ?> required>
						</div>
					</div>
					<!-- /.box-body -->
					<div class="card-footer">
						<button type="submit"
								class="btn btn-primary pull-right"><?= trans('save_changes'); ?></button>
					</div>
					<!-- /.box-footer -->
					<?= form_close(); ?><!-- form end -->
				</div>
	...
	<div class="col-md-12">
		<div class="card">
			<div class="card-header">
				<div class="pull-left">
					<h3 class="card-title"><?= trans('navigation'); ?></h3>
				</div>
			</div><!-- /.box-header -->
							...
							<table class="table table-bordered table-striped dataTable" id="cs_datatable_lang" role="grid" aria-describedby="example1_info">
								...
								<?php foreach ($menu_items as $menu_item): ?>
									<?php if ($menu_item->item_location == "header"): ?>
										<tr>
											<td><?= $menu_item->item_order; ?></td>
											<td><?= html_escape($menu_item->item_name); ?></td>
											<td>
												...
												<td>
													<div class="dropdown">
														...
														<ul class="dropdown-menu options-dropdown">
															<li>
																<a href="<?= admin_url(); ?>update-category/<?= $menu_item->item_id; ?>?redirect_url=<?= admin_url(); ?>navigation"><i class="fa fa-edit option-icon"></i><?= trans('edit'); ?></a>
															</li>
															<li>
																<a href="javascript:void(0)" onclick="delete_item('category_controller/delete_category_post','<?= $menu_item->item_id; ?>','<?= trans("confirm_category"); ?>');"><i class="fa fa-trash option-icon"></i><?= trans('delete'); ?></a>
															</li>
														</ul>
													</div>
												</td>
											<?php elseif ($menu_item->item_type == "page" && empty($menu_item->item_link)): ?>
												<td>
													<div class="dropdown">
														...
														<ul class="dropdown-menu options-dropdown">
															<li>
																<a href="<?= admin_url(); ?>update-page/<?= $menu_item->item_id; ?>?redirect_url=<?= admin_url(); ?>navigation"><i class="fa fa-edit option-icon"></i><?= trans('edit'); ?></a>
															</li>
															<li>
																<a href="javascript:void(0)" onclick="delete_item('page_controller/delete_page_post','<?= $menu_item->item_id; ?>','<?= trans("confirm_page"); ?>');"><i class="fa fa-trash option-icon"></i><?= trans('delete'); ?></a>
															</li>
														</ul>
													</div>
												</td>
											<?php else: ?>
												<td>
													<div class="dropdown">
														...
														<ul class="dropdown-menu options-dropdown">
															<li>
																<a href="<?= admin_url(); ?>update-menu-link/<?= $menu_item->item_id; ?>"><i class="fa fa-edit option-icon"></i><?= trans('edit'); ?></a>
															</li>
															<li>
																<a href="javascript:void(0)" onclick="delete_item('admin_controller/delete_navigation_post','<?= $menu_item->item_id; ?>','<?= trans("confirm_link"); ?>');"><i class="fa fa-trash option-icon"></i><?= trans('delete'); ?></a>
															</li>
														</ul>
													</div>
												</td>
