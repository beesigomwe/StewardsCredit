...
<div class="row">
	<div class="col-sm-12 col-xs-12">
		<div class="card">
			...
			<!-- form start -->
			<?= form_open('page_controller/update_page_post'); ?>

			<input type="hidden" name="id" value="<?= html_escape($page->id); ?>">
			...
			<div class="card-body">
				...
				<div class="form-group">
					...
					<input type="text" class="form-control" name="title" placeholder="<?= trans('title'); ?>"
						   value="<?= html_escape($page->title); ?>" <?= ($rtl == true) ? 'dir="rtl"' : ''; ?> required>
				</div>
				...
				<?php if ($page->is_custom == 1): ?>
					<div class="form-group">
						...
						<input type="text" class="form-control" name="slug" placeholder="<?= trans("slug"); ?>" value="<?= html_escape($page->slug); ?>" <?= ($rtl == true) ? 'dir="rtl"' : ''; ?>>
					</div>
				<?php else: ?>
					<input type="hidden" name="slug" value="<?= html_escape($page->slug); ?>">
				<?php endif; ?>

				<div class="form-group">
					<label class="control-label"><?= trans("description"); ?> (<?= trans('meta_tag'); ?>)</label>
					<input type="text" class="form-control" name="page_description"
						   placeholder="<?= trans("description"); ?> (<?= trans('meta_tag'); ?>)"
						   value="<?= html_escape($page->page_description); ?>" <?= ($rtl == true) ? 'dir="rtl"' : ''; ?>>
				</div>

				<div class="form-group">
					<label class="control-label"><?= trans('keywords'); ?> (<?= trans('meta_tag'); ?>)</label>
					<input type="text" class="form-control" name="page_keywords"
						   placeholder="<?= trans('keywords'); ?> (<?= trans('meta_tag'); ?>)" value="<?= html_escape($page->page_keywords); ?>" <?= ($rtl == true) ? 'dir="rtl"' : ''; ?>>
				</div>
				...
				<?php else: ?>
					<input type="hidden" name="parent_id" value="<?= html_escape($page->parent_id); ?>">
				<?php endif; ?>

				<div class="form-group">
					<label><?= trans('order'); ?></label>
					<input type="number" class="form-control" name="page_order" placeholder="<?= trans('order'); ?>" value="<?= html_escape($page->page_order); ?>" min="1" style="width: 300px;max-width: 100%;" <?= ($rtl == true) ? 'dir="rtl"' : ''; ?>>
				</div>
					...
					<div class="form-group">
						<div class="row">
							...
							<div class="col-md-2 col-sm-4 col-xs-12 col-option">
								<input type="radio" id="location_1" name="location" value="header" class="square-purple" <?= ($page->location == 'header') ? 'checked' : ''; ?>>
								<label for="location_1" class="cursor-pointer"><?= trans('header'); ?></label>
							</div>
							<div class="col-md-2 col-sm-4 col-xs-12 col-option">
								<input type="radio" id="location_2" name="location" value="footer" class="square-purple" <?= ($page->location == 'footer') ? 'checked' : ''; ?>>
								<label for="location_2" class="cursor-pointer"><?= trans('footer'); ?></label>
							</div>
						</div>
					</div>
				<?php else: ?>
					<input type="hidden" name="location" value="<?= html_escape($page->location); ?>">
				<?php endif; ?>
				...
				<?php else: ?>
					<input type="hidden" name="page_active" value="<?= html_escape($page->page_active); ?>">
				<?php endif; ?>
