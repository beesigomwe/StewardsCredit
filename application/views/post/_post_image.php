	...
	if (!empty($post_item->image_url) || $post_item->image_mime == 'gif' || $post_item->post_type == 'video'):?>
		<div class="external-image-container">
			<?php if ($post_item->post_type == 'video'): ?>
				<img src="<?= base_url(); ?>assets/img/icon_play.svg" alt="icon" class="post-icon <?= $icon; ?>"/>
			<?php endif; ?>
			<img src="<?= $bg; ?>" class="img-responsive" alt="<?= html_escape($post_item->title); ?>">
			<img src="<?= $bg; ?>" data-src="<?= $img_url; ?>" alt="<?= html_escape($post_item->title); ?>" class="lazyload img-external" onerror='<?= $bg; ?>'>
		</div>
	<?php else: ?>
		<img src="<?= $bg; ?>" data-src="<?= $img_url; ?>" class="lazyload img-responsive" alt="<?= html_escape($post_item->title); ?>" onerror="javascript:this.src='<?= $bg; ?>'">
	<?php endif;
endif; ?>
