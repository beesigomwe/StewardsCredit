            ...
            <?php if ($page->breadcrumb_active == 1): ?>
                <div class="page-breadcrumb">
                    <ol class="breadcrumb">
                        ...
                        <li class="breadcrumb-item">
                            <a href="<?= lang_base_url(); ?>gallery"><?= html_escape($page->title); ?></a>
                        </li>
                        <li class="breadcrumb-item active"><?= html_escape($album->name); ?></li>
                    </ol>
                </div>
            <?php else: ?>
                ...
                <div class="content page-about page-gallery">
                    ...
                    <div class="row">
                        <div class="col-sm-12 text-center">
                            <h2 class="gallery-category-title"><?= html_escape($album->name); ?></h2>
                        </div>
                    </div>
                                ...
                                <div class="filters text-center">
                                    ...
                                    <?php foreach ($gallery_categories as $category): ?>
                                        <label data-filter="category_<?= $category->id; ?>" class="btn btn-primary">
                                            <input type="radio" name="options"> <?= $category->name; ?>
                                        </label>
                                    <?php endforeach; ?>
                                </div>
                            </div>
                            ...
                            <?php foreach ($gallery_images as $item): ?>
                                <div data-filter="category_<?= $item->category_id; ?>" class="col-lg-4 col-md-4 col-sm-6 col-xs-12 gallery-item">
                                    <div class="item-inner">
                                        <a class="image-popup lightbox" href="<?= base_url() . $item->path_big; ?>" data-effect="mfp-zoom-out" title="<?= html_escape($item->title); ?>">
                                            <img src="<?= base_url() . html_escape($item->path_small); ?>" alt="<?= html_escape($item->title); ?>" class="img-responsive"/>
                                        </a>
                                    </div>
                                </div>
                            <?php endforeach; ?>
