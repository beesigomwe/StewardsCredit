...
public function userManagement($action='',$param1='')
{
   ...
   if($action=='insert'){  
    ...
    $data['role']=$this->input->post('user-type',true);  
    ...
    if (!$this->form_validation->run() == FALSE)
    {
        if($do=='insert'){ 
        //Check Duplicate Entry  
            if(!value_exists("user","username",$data['username'])){ 
            ...
            if(!value_exists("user","username",$data['username'],"id",$id)){
                if(!value_exists("user","email",$data['email'],"id",$id)){ 
        //if password change
    ...
    $this->db->delete('user', array('id' => $param1));        
}      
...
public function getUser($id)
{
    ...
    if($result->fullname == '') $result->fullname = $result->firstname.' '.$result->lastname;
    if($result->username == '') $result->username = $result->email;
...
public function uploadLogo()
{
    ...
    else
    {
        ...
        if($this->db->update('admin_settings', $object)){
            echo "true";
        }
    } 
...
public function changePassword($id = null){
   ...
   if (!$this->form_validation->run() == FALSE) {
    ...
    //update Password
        if(null == $id) $id=$this->session->userdata('id');    
        $this->db->where('id',$id);
        $this->db->update('user',$data);
        echo "true";
    }   
